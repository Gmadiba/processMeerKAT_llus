...
def write_spw_master(filename,config,SPWs,precal_scripts,postcal_scripts,submit,dir='jobScripts',pad_length=5,dependencies='',timestamp='',slurm_kwargs={}):
    ...
    header = '-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------' + '-'*pad_length
    do = r"""
    mkdir -p Reduction_archive; find . -path ./Reduction_archive -prune -o \( \( -type d -name "plots" \) -o \( -type d -name "SPW_MFSs" \) -o \( -type d -name "*science.image*" \) -o \( -name "*.mms*" ! -name "*MHz*" \) -o -name "*.png*" \) -prune -exec mv {} Reduction_archive/ \;
    for f in {%s,}; do if [ -d $f ]; then cd $f; ./%s/%s%s; cd ..; else echo Directory $f doesn\'t exist; fi; done;%s
    """
...
def write_all_bash_jobs_scripts(master,extn,IDs,dir='jobScripts',echo=True,prefix='',pad_length=5, slurm_kwargs={}):
    ...
    write_bash_job_script(master, errorScript, extn, do, r'find errors \(after pipeline has run\)', dir=dir, echo=echo)
    ...
    write_bash_job_script(master, timingScript, extn, do, r'display start and end timestamps \(after pipeline has run\)', dir=dir, echo=echo)
    ...
    write_bash_job_script(master, cleanupScript, extn, do, r'remove MSs/MMSs from this directory \(after pipeline has run\)', dir=dir, echo=echo)
